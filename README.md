...
~~~  - dimensions per KV head = 3~~~
~~~  - dimensions per Q head = dimensions per KV head * kv heads per query head (to gurantee it's dividible) = 6~~~
...
Shapes unchanged (used the nontransposed simpler layout of Q/K in the Reshaping state):
Q: [7,5,8,6]
...
- Softmax - get probalities
  ...
  - if inference next_token = sample(probs) # depending on the temperature, top-k, top-p and whatnut, but we can go for the highest for simplicity